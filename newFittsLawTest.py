...
def mousePressed(canvas, event):
    if (canvas.data.STATE == "Display_Target" or
        ...
        switchButtonPressed(canvas,event)
    redrawAll(canvas)
...
def switchButtonPressed(canvas, event):
    x1 = 50
    y1 = 50
    x2 = 80
    y2 = 80
...
#Below is for set up
        ...
        if (canvas.data.client and event.keysym=="space"):
            ...
            canvas.data.spaceDown = 1
        elif (canvas.data.client and event.keysym == canvas.data.switchKeys):
